...
ControllerLoop::ControllerLoop(Data_Xchange *data, sensors_actuators *sa,
                               Mirror_Kinematic *mk, float Ts)
...
void ControllerLoop::loop(void) {
  ...
  while (1) {
    ...
    {
      find_index();
      if (0) // index1.positionAtIndexPulse != 0 && index2.positionAtIndexPulse
             // != 0)
        is_initialized = true;
    } else {
...
void ControllerLoop::find_index(void) {
  // use a simple P-controller to get system spinning, add a constant current to
  // overcome friction
}
